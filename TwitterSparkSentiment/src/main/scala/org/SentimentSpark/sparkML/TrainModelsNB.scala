...
object TrainModelsNB {
  ...
  def createAndSaveNBModel(sc: SparkContext,
    ...
    val naiveBayesModel: NaiveBayesModel =
  ...
  def validateAccuracyOfNBModel(
      ...
      stopWordsList: Broadcast[List[String]]): Unit = {
    val naiveBayesModel: NaiveBayesModel =
      NaiveBayesModel.load(sc, PropertiesLoader.ModelPath)
      ...
      case Row(polarity: Int, tweet: String) =>
        ...
        (polarity.toDouble,
         naiveBayesModel.predict(
           ParsingTweets.transformFeatures(tweetInWords)),
         tweetText)
    }

    val actualVsPredictionRDD2 =
      tweetsDF.select("polarity", "status").rdd.map {
        case Row(polarity: Int, tweet: String) =>
          val tweetText = replaceNewLines(tweet)
          (polarity.toDouble,
    ...
    val accuracy2 = 100.0 * actualVsPredictionRDD2
        .filter(x => x._1 == x._2)
        .count() / tweetsDF.count()
    println(
